goog.provide('application.Diceros.VectorLayer');

goog.require('application.Diceros.BezierAGG');
goog.require('application.Diceros.Layer');
goog.require('application.Diceros.Point');
 ...
 * @constructor
 */
application.Diceros.VectorLayer = function(app) {
  goog.base(this, app);
  //application.Diceros.Layer.call(this, app);
   ...
   * @type {application.Diceros.VectorLayer.Mode}
   */
  this.mode = application.Diceros.VectorLayer.Mode.DEFAULT;
  /**
  ...
  /**
   * 全ての線
   * @type {Array.<application.Diceros.Line>}
   */
  this.lines = [];
  /**
   * 現在の編集中の線の index
...
goog.inherits(
  application.Diceros.VectorLayer,
 ...
 * @enum {number}
 */
application.Diceros.VectorLayer.Mode = {
 ...
 */
application.Diceros.VectorLayer.prototype.event = function(event) {
  ...
  switch (event.type) {
    ...
    case goog.events.EventType.MOUSEMOVE:
      ...
      if (this.app.getCurrentCanvasWindow().drag) {
        switch (this.mode) {
          // 線を描く
          case application.Diceros.VectorLayer.Mode.DEFAULT:
            this.sampling();
            this.refreshCurrentLineOutline();
            break;
          // 編集モードで点の移動中だったら線を描き直す
          case application.Diceros.VectorLayer.Mode.EDIT:
            if (this.currentCtrlPoint !== null) {
              ...
              currentLine.updateControlPoint(
                ...
                new application.Diceros.Point(x, y, width, true)
              );
    ...
    case goog.events.EventType.MOUSEDOWN:
      switch (this.mode) {
        case application.Diceros.VectorLayer.Mode.DEFAULT:
          ...
          line = new application.Diceros.BezierAGG();
          point = new application.Diceros.Point(x, y, this.getCurrentPenSize()); // XXX
  ...
  if (event.altKey) {
    this.mode = application.Diceros.VectorLayer.Mode.EDIT;
    this.drawCtrlPoint();
  } else {
 ...
 * サンプリングを行う
 */
application.Diceros.VectorLayer.prototype.sampling = function() {
  var currentLine = this.lines[this.currentLine],
      point =
        new application.Diceros.Point(this.x, this.y, this.getCurrentPenSize()); // XXX
 ...
 */
application.Diceros.VectorLayer.prototype.updateCursor =
function(x, y) {
 ...
 * 現在の線のみ描画し直す
 */
application.Diceros.VectorLayer.prototype.refreshCurrentLine =
function() {
  this.restore();
  this.save();
 ...
 * 現在の線のみアウトラインで描画し直す
 */
application.Diceros.VectorLayer.prototype.refreshCurrentLineOutline =
function() {
  this.outline = true;
  this.refreshCurrentLine();
  this.outline = false;
 ...
 */
application.Diceros.VectorLayer.prototype.refreshBufferNewLine =
function() {
  // back と front の更新
  this.backBuffer =
 ...
 */
application.Diceros.VectorLayer.prototype.refreshBuffer =
function(index) {
 ...
 */
application.Diceros.VectorLayer.prototype.draw =
function(drawIndex) {
  var canvas = this.canvas,
      ctx = this.ctx,
 ...
 * @private
 */
application.Diceros.VectorLayer.prototype.drawLineImpl_ =
 ...
 */
application.Diceros.VectorLayer.prototype.drawCtrlPoint =
 ...
 */
application.Diceros.VectorLayer.prototype.drawCtrlPointImpl_ =
function(ctx, lineIndex) {
  var line = this.lines[lineIndex],
 ...
 * 制御点の表示を消す
 */
application.Diceros.VectorLayer.prototype.clearCtrlPoint =
function() {
  var ctx = this.app.windows[this.app.currentCanvasWindow].overlay.ctx,
      drawFunc;
 ...
 * 描画状況の保存
 */
application.Diceros.VectorLayer.prototype.save = function() {
/*
 ...
 * 描画状況の復元
 */
application.Diceros.VectorLayer.prototype.restore = function() {
/*
  var buffer;
 ...
 * 描画状況の保存を１つ捨てる
 */
application.Diceros.VectorLayer.prototype.pop = function() {
 ...
 */
application.Diceros.VectorLayer.prototype.checkCtrlPoint =
function(x, y) {
  var ctx = this.ctx;
 ...
 */
application.Diceros.VectorLayer.prototype.getCurrentPenSize = function() {
